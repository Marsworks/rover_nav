...
struct node {
  ...
  node(grid_map::Index ind, const grid_map::GridMap &map,
       std::string elevation_layer, std::string slope_layer) {
    index = ind;
  ...
  node(grid_map::Index ind) { index = ind; }
...
bool get_path(node *cell, const geometry_msgs::PoseStamped &goal,
...
bool node_sort(node i, node j) { return i.f < j.f; }
...
void Planner3D::initialize(std::string name,
  ...
  marker_publisher =
      nh.advertise<visualization_msgs::Marker>("/explored_cells", 1, true);
  full_map_publisher =
      nh.advertise<grid_map_msgs::GridMap>("/full_gridmap", 1, true);
  filtered_map_publisher =
      nh.advertise<grid_map_msgs::GridMap>("/filtered_gridmap", 1, true);

  global_map_subscrber = nh.subscribe<grid_map_msgs::GridMap>(
      "/grid_map_pcl_loader_node/grid_map_from_raw_pointcloud", 1,
      &Planner3D::gridmap_callback, this);

  nh.param<std::string>(name + "/elevation_layer", elevation_layer,
                        "elevation");
...
void Planner3D::gridmap_callback(
    const grid_map_msgs::GridMapConstPtr &grid_map_msg_ptr) {
  ...
  // grid_map::GridMapRosConverter::toMessage(filtered_gridmap,
  // grid_map_message); filtered_map_publisher.publish(grid_map_message);

  ROS_INFO("GridMap filtered");
}

bool Planner3D::makePlan(const geometry_msgs::PoseStamped &start,
  ...
  // grid_map::GridMap &search_gridmap = full_gridmap; //full_gridmap,
  // raw_gridmap or filtered_gridmap
  ...
  while (open_list.size() && ros::ok()) {
    ...
    if (current->index(0) == goal_pos.index(0) &&
        current->index(1) ==
            goal_pos.index(1)) // Index has been choosen instead of position to
    ...
    for (auto c : children) {
      grid_map::Index temp_index(current->index(0) + c(0),
                                 current->index(1) + c(1));
      node child =
          node(temp_index, search_gridmap, elevation_layer, slope_layer);

      int pos_in_list = is_in_list(closed_list, child);
      if (search_gridmap.isValid(temp_index, "elevation") &&
          pos_in_list == -1 &&
          child.traversable) // Checking if the index is within the map
      {
        ...
        if (pos_in_list == -1 ||
            (pos_in_list >= 0 && open_list[pos_in_list].g > child.g))
